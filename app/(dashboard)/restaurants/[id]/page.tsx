...
export default function RestaurantDetailPage() {
  ...
  const setRestaurantAccess = async (status: RestaurantStatus) => {
    ...
    try {
      ...
      setDetail({ ...detail, restaurant: { ...detail.restaurant, status: previous } });
  ...
  const submitFeedbackUpdate = async () => {
    ...
    try {
        ...
        // Optimistic Update
        setStaffFeedback(prev => prev.map(p => p.id === selectedFeedback.id ? {...p, status: newStatus, admin_response: responseNote} : p));
        closeFeedbackModal();
    } catch (e) {
        console.error(e);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 pb-12">
      ...
      <div className="mt-4 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 px-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
          <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Subscription Plan</p>
          <p className="mt-2 text-2xl font-bold text-gray-900 capitalize">{restaurant.plan}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
          <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Member Since</p>
          <p className="mt-2 text-2xl font-bold text-gray-900">{restaurant.memberSince}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
          <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Total Revenue ({range})</p>
          <p className="mt-2 text-2xl font-bold text-gray-900 text-green-600">₹{totalRangeRevenue.toLocaleString("en-IN")}</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm transition-all hover:shadow-md">
          <p className="text-xs font-medium uppercase tracking-wide text-gray-500">Lifetime Revenue</p>
          <p className="mt-2 text-2xl font-bold text-gray-900">₹{detail.totalRevenueLifetime.toLocaleString("en-IN")}</p>
        </div>
      </div>

      <section className="mt-8 px-6">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Restaurant Access Controls</h3>
          <div className="flex items-center gap-3">
            <button
              disabled={pending.startsWith("restaurant")}
              onClick={() => setRestaurantAccess("active")}
              className={`px-4 py-2 font-medium rounded-lg transition-colors ${restaurant.status === "active"
            ...
            <button
              ...
              className={`px-4 py-2 font-medium rounded-lg transition-colors ${restaurant.status === "disabled"
                  ? "bg-gray-100 text-gray-400 cursor-not-allowed"
            ...
            <button
              ...
              className={`px-4 py-2 font-medium rounded-lg transition-colors ${restaurant.status === "revoked"
                  ? "bg-gray-100 text-gray-400 cursor-not-allowed"
        ...
        <div className="fixed inset-0 bg-black/50 overflow-y-auto h-full w-full flex items-center justify-center z-50">
            <div className="relative mx-auto p-5 border w-[500px] shadow-2xl rounded-xl bg-white">
                <div className="flex justify-between items-start mb-6">
                    <h3 className="text-xl font-bold text-gray-900">Manage Feedback</h3>
                    <button 
                ...
                <div className="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                    <div className="flex items-center gap-2 mb-2">
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                            ...
                            selectedFeedback.type === 'feature_request' ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-700'
                        }`}>
                            {selectedFeedback.type?.replace('_', ' ')}
                        </span>
                         <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                            selectedFeedback.priority === 'critical' ? 'bg-rose-600 text-white' : 
                            selectedFeedback.priority === 'high' ? 'bg-orange-500 text-white' : 'bg-green-100 text-green-700'
                        }`}>
                            {selectedFeedback.priority}
                        </span>
                    </div>
                    <h4 className="font-bold text-gray-900 mb-1">{selectedFeedback.title}</h4>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200 text-xs text-gray-500 flex justify-between">
                        <span>Submitted by <span className="font-medium text-gray-700">{selectedFeedback.user_name || 'Unknown'}</span></span>
                ...
                <div className="space-y-5">
                    ...
                    <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">
                            ...
                            <span className="ml-1 font-normal text-gray-400 text-xs">(Visible to staff)</span>
                        </label>
                        <textarea
                            value={responseNote}
      ...
      <div className="mt-8 px-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Revenue Trend ({range})</h3>
          <div className="h-72 w-full">
            <RevenueChart data={detail.revenueSeries} />
          </div>
        </section>
        <section className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Orders Trend ({range})</h3>
          <div className="h-72 w-full">
      ...
      <section className="mt-8 px-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Restaurant Users</h3>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center text-xs font-semibold uppercase tracking-wide text-gray-500">Status</th>
                  <th className="px-6 py-4 text-right text-xs font-semibold uppercase tracking-wide text-gray-500">Controls</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 bg-white">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                    <td className="whitespace-nowrap px-6 py-4 font-medium text-gray-900">{u.name}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{u.email}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600 capitalize">{u.role}</td>
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          disabled={pending === `user:${u.id}` || u.status === "active"}
                          onClick={() => setUserAccess(u.id, "active")}
                          className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${u.status === "active" ? "bg-gray-100 text-gray-400" : "bg-green-600 text-white hover:bg-green-700"
                            }`}
                        >
                          Activate
                        </button>
                        <button
                          disabled={pending === `user:${u.id}` || u.status === "disabled"}
                          onClick={() => setUserAccess(u.id, "disabled")}
                          className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${u.status === "disabled" ? "bg-gray-100 text-gray-400" : "bg-amber-600 text-white hover:bg-amber-700"
                            }`}
                        >
                          Disable
                        </button>
                        <button
                          disabled={pending === `user:${u.id}` || u.status === "revoked"}
                          onClick={() => setUserAccess(u.id, "revoked")}
                          className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${u.status === "revoked" ? "bg-gray-100 text-gray-400" : "bg-red-600 text-white hover:bg-red-700"
                            }`}
                        >
                          Revoke
                        </button>
      ...
      <section className="mt-8 px-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Staff Feedback</h3>
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                     <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Date/Details</th>
                            <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Source</th>
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                        sf.type === 'bug' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
                                    }`}>{sf.type}</span>
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
                                        sf.priority === 'critical' ? 'bg-rose-600 text-white' : 'bg-orange-100 text-orange-800'
                                    }`}>{sf.priority}</span>
                                    <span className="font-bold text-gray-900 text-sm ml-1">{sf.title}</span>
                                </div>
                                <p className="text-sm text-gray-600 whitespace-pre-wrap mb-1">{sf.description}</p>
                                <p className="text-[10px] text-gray-400">{new Date(sf.created_at).toLocaleDateString()}</p>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 align-top">
                                {sf.user_name || 'Unknown'} <span className="text-xs text-gray-400">({sf.user_role || 'Staff'})</span>
                              </td>
                               <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 align-top">
                                <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${
                                    sf.status === 'open' ? 'bg-green-100 text-green-800' : 
                                    sf.status === 'resolved' ? 'bg-gray-100 text-gray-800 line-through' :
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Source</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 w-1/2">Comment</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 bg-white">
                {feedback.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-8 text-center text-gray-500 text-sm">No feedback found.</td>
                  </tr>
                ) : (
                  feedback.map((f) => (
                    <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                      <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{f.createdAt}</td>
                      <td className="whitespace-nowrap px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-1 rounded-md font-bold text-xs ${f.rating >= 4 ? "bg-green-100 text-green-700" : f.rating === 3 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"
                          }`}>
                          ⭐ {f.rating}/5
                        </span>
                      </td>
                      <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600 capitalize">{f.source}</td>
                      <td className="px-6 py-4 text-sm text-gray-700 whitespace-wrap">{f.comment || <span className="text-gray-400 italic">No comment provided</span>}</td>
                    </tr>
                  ))
                )}
              </tbody>
      ...
      <section className="mt-8 px-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Downtime Log</h3>
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Start Time</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">End Time</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Duration</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500">Severity</th>
                  <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 w-1/3">Reason</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 bg-white">
                {downtimes.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-8 text-center text-gray-500 text-sm">No downtime incidents recorded.</td>
                  </tr>
                ) : (
                  downtimes.map((d) => (
                    <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                      <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{new Date(d.startedAt).toLocaleString("en-IN")}</td>
                      <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-600">{new Date(d.endedAt).toLocaleString("en-IN")}</td>
                      <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{d.minutes} mins</td>
                      <td className="whitespace-nowrap px-6 py-4">
                        <span className={`inline-flex items-center px-2 py-1 rounded-md font-bold text-xs uppercase tracking-wider
                          ${d.severity === "low" ? "bg-blue-100 text-blue-700" : d.severity === "medium" ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}
                        `}>
                          {d.severity}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700">{d.reason || <span className="text-gray-400 italic">Unspecified</span>}</td>
                    </tr>
                  ))
                )}
              </tbody>
